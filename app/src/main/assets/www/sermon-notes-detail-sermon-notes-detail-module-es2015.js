...
let SermonNotesDetailPageModule = class SermonNotesDetailPageModule {
};
...
let SermonNotesDetailPage = class SermonNotesDetailPage {
    constructor(idService, route, loader, storage, sermonsService, router, eventsService, apiService, toast, 
    ...
    getDBNote() {
        ...
        this.loader.presentLoading().then(() => {
            this.sermonsService.getCMSSermonNote(body).subscribe((res) => {
                ...
                let jq_code = self.note_detail.content.split('<script>');
                jq_code.forEach((element, index) => {
                    if (index != 0) {
                        this.createJqueryCode(element);
                    }
                });
    ...
    createJqueryCode(code) {
        if (code.includes("jQuery('.field_fill")) {
            let new_code = code.split('.field_fill');
            new_code = new_code[1].split("'");
            $(document).ready(function () {
                $('.field_fill' + new_code[0]).keyup(function () {
                    localStorage.setItem('textinput_fill' + new_code[0], $(this).val());
                    var value2 = $(this).width();
                    $(this).css('width', localStorage.setItem('width_fill' + new_code[0], ($(this).val().length * 14).toString()));
                    $(this).css('width', localStorage.getItem('width_fill' + new_code[0]));
                });
                $('.field_fill' + new_code[0]).each(function () {
                    var key = $(this).attr('id');
                    var value2 = $(this).width();
                    if (localStorage.getItem('textinput_fill' + new_code[0])) {
                        $(this).val(localStorage.getItem('textinput_fill' + new_code[0]));
                    }
                    $(this).css('width', localStorage.getItem('width_fill' + new_code[0]));
                });
                $(this).keyup(function () {
...
SermonNotesDetailPage.ctorParameters = () => [
...
//# sourceMappingURL=sermon-notes-detail-sermon-notes-detail-module-es2015.js.map