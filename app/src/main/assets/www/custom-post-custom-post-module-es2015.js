...
let CustomPostPageModule = class CustomPostPageModule {
};
...
let CustomPostPage = class CustomPostPage {
    constructor(idService, route, loader, storage, sermonsService, router, eventsService, apiService, toast, appService, location, common, event_provider, analytics, platform, device, translate) {
        ...
        if (localStorage.getItem('church_data')) {
            ...
            this.event_provider.getChurchData.subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                ...
                if (res.apppages.length > 0) {
                    let custom_post_page = res.apppages.find(x => x.id === this.page_id);
                    if (custom_post_page) {
    ...
    getDBPosts() {
        this.loader.presentLoading().then(() => {
            ...
            this.sermonsService.getCMSSermonPosts(body).subscribe((res) => {
                    ...
                    if (res.description && JSON.stringify(res.description) != '{}') {
                        if (res.description.includes("<img")) {
                            ...
                            if (res.image_url !== "" && res.image_url !== null && res.image_url.includes("s3.wasabisys.com") && Object(_angular_common__WEBPACK_IMPORTED_MODULE_10__["formatDate"])(res.created_at, 'yyyy-MM-dd', 'en_US') >= '2023-05-29') {
                                ...
                                let new_link = res.image_url.replace(folder_name, this.deviceType);
    ...
    getCategoryNotes(url) {
        this.loader.presentLoading().then(() => {
            ...
            this.sermonsService.getCategoryNotes(url, this.page_id).subscribe((res) => {
                // ======= parsing xml into JsSON format =============================
                // let parseString = require('xml2js').parseString;
                // parseString(res, function (err, result) {
                //   self.allNotes = result.rss.channel[0].item;
    ...
    openDetail(i) {
        ...
        let hashed_title = this.allNotes[i].title ? md5(this.allNotes[i].title) : ''; // hashed title before sending
...
//# sourceMappingURL=custom-post-custom-post-module-es2015.js.map