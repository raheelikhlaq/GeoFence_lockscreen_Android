...
let LiveStreamPage = class LiveStreamPage {
    // isStarted = false;
    constructor(route, loader, storage, router, apiService, dom, auth, toast, datePipe, location, appService, analytics, common, event_provider, translate, popoverController, modalController, youtube, geoLocation, device, socket, socket2, ipservice) {
        this.route = route;
        this.loader = loader;
    ...
    ionViewWillEnter() {
        ...
        this.event_provider.getChurchData.subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            if (!(Object.keys(res).length === 0)) {
                let app_pages = res.apppages.find(x => x.name === 'Live Stream');
    ...
    getLiveStream() {
        // fetching live stream data
        this.apiService.liveStream(this.ch_id).subscribe((res) => {
            if (res.status) {
                this.liveStream = res.livestream;
                if (this.liveStream.poster) {
                    this.streamPoster = this.liveStream.poster;
                }
                if (this.liveStream.schedule) {
                    let mytime = moment__WEBPACK_IMPORTED_MODULE_15__(this.liveStream.schedule);
                    ...
                    let time_diff = moment__WEBPACK_IMPORTED_MODULE_15__().utcOffset(); // in minutes
                    if (time_diff != 0) {
                        this.liveStream.schedule = this.appService.getDateTimeFormat(mytime);
                    }
                    else {
                        this.liveStream.schedule = this.appService.getDateTimeFormat(mytime);
                    }
                ...
                this.comments.map((item) => {
                    ...
                    let mytime = moment__WEBPACK_IMPORTED_MODULE_15__(item.comment_time);
                    let church_data = JSON.parse(localStorage.getItem('church_data'));
                    if (church_data.church.timezone) {
                ...
                if (this.liveStream.url != '' && this.liveStream.live_stream_type === 'video') {
                    if (this.liveStream.url.includes('facebook.com')) {
                        ...
                        //     }
                        // });
                        this.liveStream.url = "https://www.facebook.com/plugins/video.php?href=" + this.liveStream.url + "&show_text=false&height=375&appId";
                    }
                    else if (this.liveStream.url.includes('youtube')) {
                        this.is_youtube_video = true;
                        if (this.liveStream.url.includes('watch?v=')) {
                            this.liveStream.url = this.liveStream.url.replace('watch?v=', 'embed/');
                        }
                        else {
                            this.liveStream.url = this.liveStream.url.replace('youtube.com', 'youtube.com/embed');
                        }
                    }
                    else if (this.liveStream.url.includes('youtu.be')) {
                        this.liveStream.url = this.liveStream.url.replace('youtu.be/', 'www.youtube.com/embed/');
                    }
                    else if (this.liveStream.url.includes('vimeo')) {
                        if (this.liveStream.url.includes('www.vimeo')) {
                            this.liveStream.url = this.liveStream.url.replace('www.vimeo.com/', 'player.vimeo.com/video/');
                        }
                        else if (this.liveStream.url.includes('player.vimeo')) {
                            this.liveStream.url.includes = this.liveStream.url.replace('player.vimeo', 'player.vimeo');
                        }
                        else {
                            this.liveStream.url = this.liveStream.url.replace('vimeo.com/', 'player.vimeo.com/video/');
                        }
                    }
                    else if (this.liveStream.url.includes('yourstreamlive.com')) {
                        ...
                        this.liveStream.url = `${this.liveStream.url}?responsive=yes`;
                    }
                }
                if (this.liveStream.url != '' && this.liveStream.live_stream_type === 'audio') {
                    if (this.liveStream.url.includes("soundcloud.com")) {
                        this.soundcloud = true;
                        this.liveStream.url = "https://w.soundcloud.com/player/?url=" + this.liveStream.url;
                    }
                }
                this.liveStream.url = this.liveStream.url;
                this.liveUrl = this.dom.bypassSecurityTrustResourceUrl(this.liveStream.url);
                ...
                this.liveStreamData = res;
                console.log("livestream Data", this.liveStreamData, "------", this.liveStream);
                if (this.liveUrl !== "") {
                    this.isStarted = true;
                }
                if (moment__WEBPACK_IMPORTED_MODULE_15__(this.liveStream.schedule.toString()).isSameOrBefore(moment__WEBPACK_IMPORTED_MODULE_15__())) {
                    this.isStarted = true;
                    clearInterval(this.liveStreamingInterval);
                }
                else {
                    this.isStarted = false;
                    this.isLiveStreamingStarted();
                }
    ...
    isLiveStreamingStarted() {
        this.liveStreamingInterval = setInterval(() => {
            if (moment__WEBPACK_IMPORTED_MODULE_15__(this.liveStream.schedule.toString()).isSameOrBefore(moment__WEBPACK_IMPORTED_MODULE_15__())) {
                this.isStarted = true;
                clearInterval(this.liveStreamingInterval);
                // this.getLiveStream();
    ...
    getUser() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            this.userData = yield this.auth.getUser();
    ...
    setAnalytics() {
        let end_time_analytics = moment__WEBPACK_IMPORTED_MODULE_15__();
    ...
    createComment() {
        if (this.userLoggedIn) {
            ...
            this.loader.presentLoading().then(() => {
                // saving comment data
                this.apiService.create_liveStream_comment(body).subscribe((res) => {
        ...
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            popover.onDidDismiss().then(res => {
                if (res.data && res.data.event) {
                    if (this.comment_input === undefined) {
                        this.comment_input = '';
                    }
                    this.comment_input += res.data.event.char;
                }
            });
    ...
    sendLiveStreamingAnalytics() {
        ...
        this.analytics.setliveStreamingAnalytics(body).subscribe((res) => {
        });
    }
};
...
//# sourceMappingURL=live-stream-live-stream-module-es2015.js.map