...
let NewsletterDetailPageModule = class NewsletterDetailPageModule {
};
...
let NewsletterDetailPage = class NewsletterDetailPage {
    constructor(storage, route, loader, api, idService, router, eventsService, apiService, toast, location, common, event_provider, analytics, platform, device, translate, dom, modalController) {
    ...
    getNewsLetterbyid() {
        this.api.getnewsletterbyid(this.ch_id, this.post_id).subscribe((res) => {
            if (res && res.status) {
                this.post = res.posts;
                if (this.post.subject !== '' && this.post.subject !== null) {
                    this.post['subjects_array'] = JSON.parse(this.post.subject);
                    // ZB changes
                }
                if (this.post.attachment !== "" && this.post.attachment !== null && this.post.attachment.includes("s3.wasabisys.com") && Object(_angular_common__WEBPACK_IMPORTED_MODULE_9__["formatDate"])(this.post.created_at, 'yyyy-MM-dd', 'en_US') >= '2023-05-29') {
                    let abc = this.post.attachment.split("/");
                    //  let img_name = abc[abc.length-1];
                    let folder_name2 = abc[abc.length - 2];
                    if (this.platform.is("mobile")) {
                        ...
                        this.deviceType = folder_name2;
                    }
                    let new_link = this.post.attachment.replace(folder_name2, this.deviceType);
                    this.post.attachment = new_link;
                    this.post['folder_name'] = folder_name2;
                }
...
NewsletterDetailPage.ctorParameters = () => [
...
//# sourceMappingURL=newsletter-detail-newsletter-detail-module-es2015.js.map