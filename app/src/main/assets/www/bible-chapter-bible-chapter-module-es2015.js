...
let BibleChapterPageModule = class BibleChapterPageModule {
};
...
;
let BibleChapterPage = class BibleChapterPage {
    constructor(location, router, route, _bibleService, appService, loader, toast, storage, sermonsService, alertController, chatService, 
    ...
    event_provider, translate, apiService, dom, platform, socialSharing, common, audioService, el, zone, analyticsFirebase) {
        ...
        if (localStorage.getItem('bible')) {
            let b = JSON.parse(localStorage.getItem('bible'));
            if (b.language) {
    ...
    countStartTeachings() {
        ...
        this.teachingCounterId = setTimeout(() => {
            this.countStartTeachings();
        }, 1000);
    }
    // to play next verse
    playNextVerse() {
        if (this.selected_page !== 'sermon-detail') {
            ...
            this.loader.presentLoading().then(loader => {
            ...
            if (this.mobileView) {
                ...
                setTimeout(() => {
                    ...
                    if (obj.selected_audio < parseInt(obj.book.number_of_chapters)) {
                        ...
                        this.display = false;
                        this.leftPosition = 1000;
                        let required_book_order = parseInt(obj.book.book_order) + 1;
                ...
                if (this.selected_audio < parseInt(this.book.number_of_chapters)) {
                    ...
                    this.loaded = false;
                    let required_book_order = parseInt(this.book.book_order) + 1;
                    //  this.setBiblePageNo('next', required_book_order);
                    this.setCMPBiblePageNo('next', required_book_order);
                }
            }
            ...
            if (this.mobileView) {
                ...
                setTimeout(() => {
                    ...
                    if (obj.selected_audio > 1) {
                        ...
                        this.displayScene = true;
                        this.displayPrevious = false;
                        let required_book_order = parseInt(obj.book.book_order);
    ...
    getCopyrightInfo(dam_id) {
        ...
        this._bibleService.copyrightTextes(src_environments_environment__WEBPACK_IMPORTED_MODULE_19__["environment"].church_id).subscribe((res) => {
            //console.log(res,":get result")
            if (res.status) {
                this.audio_copyright = res.data.audio_copyright;
                this.text_copyright = res.data.text_copyright;
                this.content_copyright = res.data.content_copyright;
            }
        });
    }
    setBiblePageNo(type, book_order) {
        var obj = this;
        this._bibleService.getCMPbibleBooks(this.dam_id).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            if (res) {
                if (res && res.length > 0) {
                    let book = res.find(x => parseInt(x.bookOrder) === parseInt(book_order));
                    if (book) {
                        let chapter = {
    ...
    setCMPBiblePageNo(type, book_order) {
        let obj = this;
        ...
        this._bibleService.getCMPbibleBooks(this.dam_id).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (res) {
                if (res && res.length > 0) {
                    ...
                    let book = data.find(x => parseInt(x.book_order) === parseInt(book_order));
                    if (book) {
                        let chapter = {
                            id: (type === 'prev' ? parseInt(book.number_of_chapters) : 1),
                            selected: true
    ...
    ionViewDidEnter() {
        ...
        this.storage.get('BibleFontSize').then((res) => {
            if (res) {
                this.fontSize = res;
            }
            else {
        ...
        let interval = setInterval(() => {
            // this.ngOnInit();
            if (this.selected_page === 'N') {
                if (this.from && this.from === 'header') {
                }
                else {
                    this.location.back();
                }
        ...
        this.translate_event = this.event_provider.openModalTranslation.subscribe(value => {
            if (!this.modal_is_on) {
                this.modal_is_on = true;
    ...
    getParams() {
        this.route.queryParams.subscribe(params => {
            if ((Object.keys(params).length === 0)) {
                params = this._bibleService.page;
            }
            console.log(params);
            if (params && params.chapter && params.book) {
                this.chapter = JSON.parse(params.chapter);
                this.book = JSON.parse(params.book);
                this.dam_id = params.dam_id;
                this.bible_chapter_audio_id = params.audio_id;
                this.page_id = params.page_id;
                this.sermon_id = params.sermonid;
                this.page = params.page;
                this.start_chapter = params.start_chapter,
                    this.start_verse = params.start_verse,
                    this.end_chapter = params.end_chapter,
                    this.end_verse = params.end_verse,
                    this.bible_id = params.bible_id,
                    // console.log('chapter audio id is: ',this.bible_chapter_audio_id);
                    console.log(params.page);
                this.selected_page = params.page;
                this.type = params.type;
                this.from = params.from;
                this.allChapters = params.allChapters ? params.allChapters : this.book.chapters;
                this.langCode = localStorage.getItem('app_language');
                this.storage.get('app_language').then((res) => {
                    // console.log('app language data is: ', res);
                    this.langCode = res.code;
                    this.copright_info_damn_id = res.dam_id;
                });
                console.log(this.allChapters);
                let data = this.allChapters.split(',');
                //   let finalData: any = [];
                ...
                console.log(params);
                if (params.data) {
                    this.search_data = JSON.parse(params.data);
                }
            ...
            else if (this.selected_page !== 'N' && this.selected_page !== 'O') {
                // getting lang
                const l = localStorage.getItem('app_language');
                if (l) {
                    let lang = l;
                    let resp = localStorage.getItem('bible') ? JSON.parse(localStorage.getItem('bible')) : '';
                    if (resp && resp.language && !resp.language.selection_type) {
                        let bible_lang = resp.language.language_family_code;
                        this.select_book_ism(bible_lang, true);
                    }
                    else {
                        if (lang == 'Spanish') {
                            this.select_book_ism("SPN", false);
                        }
                        else {
                            this.select_book_ism("ENG", false);
                        }
                    }
                }
                else {
    ...
    // }
    setBibleByLanguage(dam_id) {
        let prev_dam_id_page_type = this.dam_id[6];
        let new_dam_id = dam_id + prev_dam_id_page_type;
    ...
    getLang_Version() {
        this.storage.get('bible_languages').then((langs) => {
            if (langs) {
                this.allLangs = langs;
    ...
    select_book_ism(lang_code, selection_type) {
    ...
    getLangsandSaveLang(lang_code, vol, selection_type) {
        this.storage.get('bible_languages').then((langs) => {
            if (langs) {
                let language_families = langs;
    ...
    saveLastReading() {
        let current_url = new URL(window.location.href);
        let complete_path = current_url.href;
        ...
        this._bibleService.saveLastOff(body).subscribe((res) => {
        });
    }
    getUserVerses() {
        ...
        this.storage.get('user').then((val) => {
            if (val != null && val != 'guest') {
                let donor_id = val.user.id;
                this.ch_id = val.user.id_church;
                // getting saved verses from API through Service
                this._bibleService.getVerses(donor_id, this.ch_id).subscribe((res) => {
                    if (res.status) {
                        this.highlighted_verses = res.verses;
                    }
                });
            }
        });
    ...
    openModal() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (this.is_cordova) {
                let share_data = '"' + this.selected_verse + '" ' + this.book.book_name + ' ' + this.chapter.id + ':' + this.selected_verse_id;
                this.socialSharing.share(share_data).then(data => {
                }, error => {
                ...
                this.socialSharing.share(this.selected_verse).then((res) => {
                }, (err) => {
                });
                // const modal = await this.modalController.create({
    ...
    getAudio(from) {
        if (this.type === "search") {
            let dam_id = this.search_data.dam_id.replace("ET", "DA");
            // getting chapter audio from API through Service
            this._bibleService.getBookChapterAudio(dam_id, this.search_data.book_id).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (res) {
                    this.chapterAudio = res.find(x => x.chapter_id === this.search_data.chapter_id);
            ...
            this._bibleService.getBookChapterAudio(dam_id, this.book.book_id, this.bible_chapter_audio_id).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (res) {
                    if (from != 'oninit getAudio') {
                        this.getChapterText('get audio else');
                    }
                    console.log(this.chapter);
                    this.chapterAudio = res.find(x => parseInt(x.number) === parseInt(this.chapter.id));
                    if (this.chapterAudio && this.chapterAudio.url) {
                        this.audioUrl = this.chapterAudio.url;
                        // this.start('chapter', this.audioUrl);
                    }
    ...
    getChapterText(from) {
        ...
        if (this.mobileView) {
            ...
            if (this.type === "search") {
                ...
                this._bibleService.getChapterText(this.search_data.dam_id, this.search_data.book_id, this.search_data.chapter_id).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                    if (res) {
                        this.verses = res;
                        this.scrollToTop(); // to scroll view from 1st verse
                    }
                ...
                if (this.page === 'sermon-detail' || this.page === 'today-study-card' || this.page === 'journey' || this.page === "study-download-detail") {
                    ...
                    this._bibleService.getStartandEndVersewithChapter(data).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        if (res.length > 0) {
                            this.verses = res;
                            setTimeout(() => {
                                this.loaded = true;
                    ...
                    this._bibleService.getChapterText(dam_id, this.book.book_id, this.book.book_id + '.' + this.chapter.id, 'ml').subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        if (res) {
                            this.verses = res;
                            setTimeout(() => {
                                this.loaded = true;
            ...
            if (this.type === "search") {
                ...
                if (this.page === 'sermon-detail' || this.page === 'today-study-card' || this.page === 'journey' || this.page === "study-download-detail") {
                    ...
                    let dam_id = this.dam_id;
                    // getting chapter text from API through Service
                    this._bibleService.getChapterText(dam_id, this.book.book_id, this.book.book_id + '.' + this.chapter.id, this.book.lang_code).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
    ...
    //         })
    // }
    getCMPNextChapterText() {
        let dam_id = this.dam_id;
        // getting chapter text from API through Service
        let chap_id = Number(this.chapter.id) + 1;
        // this.allChapters.forEach((elem) => {
        // })
        this._bibleService.getChapterText(dam_id, this.book.book_id, this.book.book_id + '.' + chap_id, this.book.lang_code).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            if (res) {
                this.nextChapterSavedVerses = res;
                setTimeout(() => {
                    this.loaded = true;
                ...
                this.scrollToTop(); // to scroll view from 1st verse
                // ====================== assigning the selected verse=============
                for (let i = 0; i < this.nextChapterSavedVerses.length; i++) {
    ...
    //         }
    // }
    getCMPpreviousChapterText() {
        let dam_id = this.dam_id;
        // getting chapter text from API through Service
        let chap_id = Number(this.chapter.id) - 1;
        if (chap_id > 0) {
            this.firstChapter = false;
            this._bibleService.getChapterText(dam_id, this.book.book_id, this.book.book_id + '.' + chap_id, this.langCode).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (res) {
                    // this.loader.stopLoading();
                    this.prevChapterSavedVerses = res;
                    setTimeout(() => {
                    ...
                    this.scrollToTop(); // to scroll view from 1st verse
                    // ====================== assigning the selected verse=============
                    for (let i = 0; i < this.prevChapterSavedVerses.length; i++) {
    ...
    save_message() {
        ...
        this.chatService.savebiblemessage(body).subscribe(res => {
            // emitting event to socket
            //this.socket.emit('send-message', {text: this.message});
            this.message = '';
            this.loadMessages(this.selected_verse_obj);
            return true;
        }, (err) => {
    ...
    loadMessages(verse) {
        ...
        this.chatService.get_bible_messages(body).subscribe((res) => {
            if (res) {
                this.current_verse_messages = res.message;
            }
            // emitting event to socket
            // this.socket.emit('send-message', {text: this.message});
            // this.load_messages();
    ...
    getTeachingVideo(book_name, chap_id, verse_id, book_name_eng) {
        ...
        if (book_name && verse_id && chap_id) {
            ...
            this._bibleService.getBibleTtbTeachingAudio(body).subscribe((res) => {
                if (res) {
                    // this.stopAudioForTtb();
                    this.ism_teachings = res;
                    //   this.verseTitle = res.result[0].name; 
                    this.current_duration_teaching = 0;
                    this.current_total_duration_teaching = 0;
                    this.progress_teachings = 0;
                    // this.range_teachings.value = 0;
                    this.ttb_teaching_data = this.ism_teachings.length ? this.ism_teachings[0] : '';
                    this.teaching_audio = this.ttb_teaching_data ? src_environments_environment__WEBPACK_IMPORTED_MODULE_19__["environment"].cmp_audioLink + this.ttb_teaching_data.uuid : '';
                    ...
                    if (localStorage.getItem('teaching_images')) {
                        if (book_name_eng !== null) {
                            ...
                            let found = images.find(x => x.bible_book_eng === book_name_eng);
                            this.ttb_bible_badge = this.ttb_teaching_data && found ? found.bible_badge_image : '../../assets/img/thrubible.PNG';
                        }
                    }
                    else {
    ...
    //         }
    // }
    getSermonsFromWordpress(base_url, verse) {
        let chap = verse.chapter_id;
        let book = verse.book_name;
        let current_verse = verse.verse_id;
        let search_book = book + ' ' + chap + ':' + current_verse;
        let self = this;
        let allSermons;
        ...
        this.sermonsService.getAllSermons(base_url).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (allSermons.length != 0) {
                let sermon;
                //=========================== filtering sermons who are teachings ========================
                // let filter_sermon = allSermons.filter(x => {
                //     if ((x.series.includes('teaching') || x.series.includes('Teaching')) && (x.hasOwnProperty('bible') && ( x.bible.includes(search_book) || x.bible.includes(search_book.toLowerCase())))) {
                //         return x;
                //     }
                //     }
                // });
        ...
        this.storage.get('user').then((val) => {
            if (val != null && val != 'guest') {
                ...
                const body = JSON.stringify({
                    ch_id: this.ch_id,
                    donor_id: donor_id,
                    book_order: this.selected_verse_obj.book_order,
                    chap_id: this.selected_verse_obj.chapter_id,
                    book_id: this.selected_verse_obj.book_id,
                    verse_id: this.selected_verse_obj.verse_id
                });
                //  === saving highlighted verse from API through Service ======================================
                this._bibleService.saveVerse(body).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                    if (res.status) {
                        this.getUserVerses();
                ...
                this.toast.presentToastWithOptions((this.alertMessage ? this.alertMessage.LoginToHighlight : 'Login to Highlight Verse'), (this.alertMessage ? this.alertMessage.WarningMessage : 'Warning Message'), "warning");
                return;
            }
            // =================================================================================================
        });
    ...
    //         );
    //   }
    gotoCopyrightPage(text) {
        this.router.navigate(["privacy-detail", { data: text }]);
    }
    playbible(isplay, audioUrl2, from) {
        if (from === 'bible') {
            if (this.audioService.audioUrl == audioUrl2) {
                ...
                setTimeout(() => {
                    this.audioService.clear();
                    this.audioService.image = this.ttb_bible_badge;
                    this.audioService.audioUrl = audioUrl2;
                    this.chapterAudio.image = this.ttb_bible_badge;
                    this.chapterAudio.title = this.book.book_name_eng + ' ' + this.chapterAudio.number;
                    this.common.audioData = this.chapterAudio;
                    this.audioService.title = this.book.book_name_eng + ' ' + this.chapterAudio.number;
                    this.common.method = "bible";
                    this.audioService.fromPage = 'bible';
                    this.audioService.liveStreaming = false;
                    this.audioService.title = this.book.book_name_eng + ' ' + this.chapterAudio.number;
                    this.audioService.bible_playing[audioUrl2] = true;
                    //   this.audioService.fromPage = "study";
                    this.audioService.bible_index = audioUrl2;
                    this.audioService.startAudio(audioUrl2);
                }, 500);
            }
        }
        else if (from === 'study') {
            if (this.audioService.audioUrl == audioUrl2) {
                ...
                setTimeout(() => {
                    ...
                    // this.common.audioData = this.chapterAudio;
                    this.audioService.title = this.ttb_teaching_data.name;
                    this.common.method = "bible";
                    this.audioService.fromPage = 'bible';
                    this.audioService.liveStreaming = false;
                    this.audioService.bible_playing[audioUrl2] = true;
...
BibleChapterPage.ctorParameters = () => [
...
let SwipeDirective = class SwipeDirective {
    ...
    ngAfterViewInit() {
        ...
        this.renderer.listen(this.elRef.nativeElement, 'touchstart', (event) => {
            this.touchPos = event.changedTouches[0].clientY;
            this.handleTouchStart(event);
            if (event.touches.length == 2) {
                this.startDifference = Math.hypot(event.touches[0].pageX - event.touches[1].pageX, event.touches[0].pageY - event.touches[1].pageY);
        ...
        this.renderer.listen(this.elRef.nativeElement, 'touchend', (event) => {
        ...
        var i;
        this.renderer.listen(this.elRef.nativeElement, 'touchmove', (event) => {
            ...
            console.log(this.isScaler);
            if (!this.isScaler) {
                if (timeDiff > 120) {
                    let newTouchPos = event.changedTouches[0].clientY;
                    if (this.touchPos < newTouchPos) {
                        this.touchPos = newTouchPos;
                        this.scrollUp.emit('up');
                    }
                    if (this.touchPos > newTouchPos) {
                        this.touchPos = newTouchPos;
                        this.scrollDown.emit('down');
                    }
                }
            }
        });
    ...
    pinchStart() {
        ...
        if (this.isScaler) {
            this.renderer.listen(this.elRef.nativeElement, 'touchmove', (event) => {
                ...
                if (this.startDifference < dist) {
                    this.startDifference = dist;
                    let fontSize = this.startDifference / this.startDifference;
                    this.pinchIn.emit(fontSize);
                }
                else if (this.startDifference > dist) {
                    this.startDifference = dist;
                    let fontSize = this.startDifference / this.startDifference;
                    this.pinchOut.emit(fontSize);
                }
            });
        }
    }
...
//# sourceMappingURL=bible-chapter-bible-chapter-module-es2015.js.map