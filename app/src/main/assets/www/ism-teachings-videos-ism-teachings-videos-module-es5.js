...
var IsmTeachingsVideosPageModule = /** @class */ (function () {
    function IsmTeachingsVideosPageModule() {
    }
    ...
    return IsmTeachingsVideosPageModule;
}());
...
var IsmTeachingsVideosPage = /** @class */ (function () {
    function IsmTeachingsVideosPage(route, location, apiService, dom, router, event_provider, storage, audioService, service, downloadService, platform, actionSheetController, socialSharing, toast, modalController) {
        this.route = route;
    ...
    IsmTeachingsVideosPage.prototype.ionViewDidEnter = function () {
        // this.event_provider.hidefooter(true, '');
        this.event_provider.hidefooter(true, 'other');
    };
    IsmTeachingsVideosPage.prototype.ngOnInit = function () {
    ...
    IsmTeachingsVideosPage.prototype.getTtbLanguageAudios = function (first_load, event) {
        var _this = this;
        var body;
            ...
            if (res.bible_id) {
                ...
                _this.apiService.getTtbLanguageAudios(body).subscribe(function (res) {
                    if (res && res.length) {
                        ...
                        _this.videos_loaded = true;
                        _this.total_page_in_pagination = res.total_pages;
                        _this.setCmpData(first_load, res);
                        _this.total_page_in_pagination = res.total_pages;
                        // let ttb_audios = res;
    ...
    // }
    IsmTeachingsVideosPage.prototype.setCmpData = function (first_load, res) {
        var _this = this;
        var ttb_audios = res;
        ...
        // }
        var read_audios = JSON.parse(localStorage.getItem("listenStudy"));
        read_audios = read_audios ? read_audios.filter(function (x) { return x.bookname == ttb_audios[0].bible_book && x.completeListner == true; }) : [];
        if (read_audios && read_audios.length > 0) {
    ...
    IsmTeachingsVideosPage.prototype.intoMinutes = function (second) {
    ...
    IsmTeachingsVideosPage.prototype.playStudy = function (audioData, index, isplay, duration) {
        var _this = this;
        this.audioService.clear();
        setTimeout(function () {
            _this.audioService.clear();
            _this.audioService.tt_all_audios = _this.ttb_audios;
            console.log(_this.book_name, " book name ", _this.book_name_eng);
            _this.index = index;
            var audio = src_environments_environment__WEBPACK_IMPORTED_MODULE_10__["environment"].cmp_audioLink + audioData.uuid;
            if (isplay == true) {
                _this.audioService.study_ttbAudios[index] = false;
                _this.audioService.togglePlayer(isplay);
            }
            else {
                if (_this.audioService.audioIndex == index) {
                    _this.audioService.study_ttbAudios[index] = true;
                    if (_this.audioService.audioUrl == audio) {
                        _this.audioService.togglePlayer(isplay);
                    }
                    else {
                        if (_this.audioService.fromPage && _this.audioService.fromPage == "ism-study") {
                            _this.audioService.heardStudy(_this.audioService.current_duration, _this.service.audioData, 'ism-study');
                        }
                        ;
                        ...
                        _this.audioService.image = audioData.art_url;
                        // this.audioService.isAudioPlayShows = !pause;
                        _this.audioService.liveStreaming = false;
                        audioData.title = audioData.name;
                        audioData.book_id = _this.book_id;
                        audioData.image = audioData.art_url;
                        _this.service.method = 'ism-study';
                        _this.audioService.index = index;
                        _this.audioService.audioUrl = audio;
                        _this.service.audioData = audioData;
                        _this.audioService.bibleBook_name = _this.book_name;
                        _this.audioService.audioUrl = audio;
                        _this.audioService.fromPage = 'ism-study';
                        audioData.seekto > 0 ? _this.audioService.startAudio(audio, audioData.seekto) : _this.audioService.startAudio(audio);
                    }
                }
                else {
                    if (_this.audioService.fromPage && _this.audioService.fromPage == "ism-study") {
    ...
    IsmTeachingsVideosPage.prototype.ionViewDidLeave = function () {
        // this.audioService.setLiveSteam();
    };
    IsmTeachingsVideosPage.prototype.getVideos = function (first_load, event) {
        var _this = this;
        ...
        this.apiService.getBibleVideoByBookId(body).subscribe(function (res) {
            if (res && res.status === true) {
                ...
                _this.data_ended = videos.length < 10;
                    ...
                    if (_this.page_type === 'bible') {
                        ...
                        if (item.vimeo_video_link) {
                            if (item.vimeo_video_link.includes('vimeo.com')) {
                                var vid_id = item.vimeo_video_link.match(/\d+/)[0];
                                var url = "https://player.vimeo.com/video/" + vid_id;
                                item['video_url'] = _this.dom.bypassSecurityTrustResourceUrl(url);
                                item.not_vimeo = false;
                            }
                            else {
                                item['video_url'] = _this.dom.bypassSecurityTrustResourceUrl(item.vimeo_video_link);
                                item.not_vimeo = true;
                            }
                        }
                        else {
                            item['video_url'] = item.original_app_video_address ? _this.dom.bypassSecurityTrustResourceUrl(item.original_app_video_address + '#t0.2') : '';
                            item.not_vimeo = true;
                        }
                    }
                    else {
                        ...
                        if (item.guid) {
                            if (item.guid.includes('vimeo.com')) {
                                var vid_id = item.guid.match(/\d+/)[0];
                                var url = "https://player.vimeo.com/video/" + vid_id;
                                item['video_url'] = _this.dom.bypassSecurityTrustResourceUrl(url);
                                item.not_vimeo = false;
                            }
                                ...
                                if (item.guid.includes('wasabisys')) {
                                    _this.apiService.checkVideo(item.guid).subscribe(function (res) {
                                        item['video_url'] = _this.dom.bypassSecurityTrustResourceUrl(item.guid + '#t0.6');
                                        item.not_vimeo = true;
                                    }, function (error) {
                                        if (error.status === 200) {
                                            item['video_url'] = _this.dom.bypassSecurityTrustResourceUrl(item.guid + '#t0.6');
    ...
    IsmTeachingsVideosPage.prototype.openVideo = function (book_id, chapter_id, verse, index, video_url, id, video) {
        if (video_url != '') {
    ...
    IsmTeachingsVideosPage.prototype.loadData = function (event) {
    ...
    IsmTeachingsVideosPage.prototype.gotobackPage = function () {
        this.location.back();
    };
    IsmTeachingsVideosPage.prototype.getTeachingImagesCache = function (book_name) {
        var _this = this;
            ...
            this.apiService.getTeachingImages().subscribe(function (res) {
                if (res && res.status) {
                    ...
                    if (book_name !== null) {
                        ...
                        var images = JSON.parse(localStorage.getItem('teaching_images'));
                        var found = images.find(function (x) { return x.bible_book_eng === book_name; });
                        _this.ttb_image = found ? found.bible_badge_image : "https://devmanage.radiobase.org/uploads/thrubible.PNG";
                    }
                }
            });
        }
    };
    IsmTeachingsVideosPage.prototype.toggleDescription = function () {
        this.showFullDescription = !this.showFullDescription;
    };
    IsmTeachingsVideosPage.prototype.studyRead = function (read) {
        ...
        if (listenData && listenData.length > -1) {
            ...
            if (idx > -1) {
                var idxs = this.ttb_audios.findIndex(function (x) { return x.uuid == listenData[idx].id; });
                if (idxs > -1) {
    ...
    IsmTeachingsVideosPage.prototype.seek = function () {
        var _this = this;
    ...
    IsmTeachingsVideosPage.prototype.shareURL = function (audio) {
            ...
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        this.socialSharing.share("Study Thru The Bible - " + audio.name + " \n" + src_environments_environment__WEBPACK_IMPORTED_MODULE_10__["environment"].cmp_audioLink + audio.uuid).then(function (data) {
                        }, function (error) {
                        });
                        return [3 /*break*/, 4];
    ...
    IsmTeachingsVideosPage.prototype.downloadCompoent = function (video) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var modal;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
    ...
    IsmTeachingsVideosPage.ctorParameters = function () { return [
...
//# sourceMappingURL=ism-teachings-videos-ism-teachings-videos-module-es5.js.map