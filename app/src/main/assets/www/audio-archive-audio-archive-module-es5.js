...
var AudioArchivePageModule = /** @class */ (function () {
    function AudioArchivePageModule() {
    }
...
var AudioArchivePage = /** @class */ (function () {
    function AudioArchivePage(location, storage, loader, sermonsService, router, toast, common, appService, analytics, dom, event_provider, platform, socialSharing, device, translate, route, modalController, popoverController, downloadService, file, transfer, downloads, casting) {
        var _this = this;
        ...
        //   }
        // });
        this.event_provider.lastSeekValue.subscribe(function (res) {
            _this.resumableArray[_this.cardIndex].seekValues.splice(res.index, 1, { value: res.value, name: res.name, progress: res.progress, duration: res.duration, pubDate: res.pubDate });
            localStorage.setItem('resumableValues', JSON.stringify(_this.resumableArray));
        });
    }
    AudioArchivePage.prototype.ngOnInit = function () {
        var _this = this;
    ...
    AudioArchivePage.prototype.openSharingModal = function (url, index) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            ...
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                ...
                if (this.isSharingActive[index] === true) {
                    this.socialSharing.share(url).then(function (res) {
                        if (res) {
                            _this.isSharingActive[index] === false;
                        }
                    }, function (err) {
                        if (err) {
                            _this.isSharingActive[index] === false;
    ...
    AudioArchivePage.prototype.hasAudioLink = function (link) {
        if (this.ch_id == "1124") {
    ...
    AudioArchivePage.prototype.preparePageData = function () {
        var _this = this;
        ...
        this.event_provider.getChurchData.subscribe(function (res) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
            var app_pages, event_type, found, i;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                if (!(Object.keys(res).length === 0)) {
                    ...
                    if (app_pages) {
                        ...
                        event_type = app_pages.custom_category_url;
                        ...
                        if (JSON.parse(localStorage.getItem('resumableValues'))) { //setting default seek values from localStorage
                            ...
                            this.resumableArray.forEach(function (e, i) {
                                if (JSON.parse(localStorage.getItem('resumableValues'))[i].id === _this.page_id) {
                                    _this.resumableArray.splice(i, 1, JSON.parse(localStorage.getItem('resumableValues'))[i]);
                                }
                            });
                            found = false;
                            for (i = 0; i < this.resumableArray.length; i++) { //check if page id already exist or not
    ...
    AudioArchivePage.prototype.getDBAudio = function () {
        var _this = this;
            ...
            self.sermonsService.getCMSAudioVideo(data).subscribe(function (res) {
                if (res.status) {
                    ...
                    if (self.allSermons.length != 0) {
                        ...
                        self.allSermons = self.allSermons.filter(function (x) { return x.sermon_audio && x.sermon_audio != ""; });
                        self.allSermons.map(function (y) {
                            y["audio_link"] = _this.dom.bypassSecurityTrustUrl(y.sermon_audio);
                        ...
                        _this.toast.presentToastWithOptions(_this.alertMessage
                            ? _this.alertMessage.NoAudio
                            : "No Audio Available", _this.alertMessage
    ...
    AudioArchivePage.prototype.getRss = function () {
        var _this = this;
        this.loader.presentLoading().then(function () {
            var self = _this;
            var data = {
            ...
            // fetching data through service
            self.sermonsService.getPodsDataWithPagintion(JSON.stringify(data)).subscribe(function (res) {
                _this.fetchMethod = 'rss';
                if (res.audio) {
                    // Assigning data to variable
                    _this.allSermons = res.audio;
                    _this.total_page_in_pagination = res.pages;
                    if (self.allSermons.length != 0) {
                        // filtering out data which have URL
                        self.allSermons = self.allSermons.filter(function (x) { return x.enclosure && x.enclosure["@attributes"].url != ""; });
                        self.allSermons.map(function (y) {
                            y["audio_link"] = _this.dom.bypassSecurityTrustUrl(y.enclosure["@attributes"].url);
                        ...
                        self.newFeeds.map(function (x) {
                            ...
                            x.comments.map(function (item) {
                                try {
                                    item.comment = JSON.parse(item.comment);
                                    var localTimeZone = moment__WEBPACK_IMPORTED_MODULE_13__["tz"].guess(true);
    ...
    AudioArchivePage.prototype.loadData = function (event) {
        var _this = this;
        ...
        if (this.fetchMethod === 'wordpress') { //pagination check if method is from world press
            if (this.page <= this.total_page_in_pagination) {
                this.sermonsService.getAllSermons(this.sermon_url, this.page).subscribe(function (res) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
                    var data;
                    var _this = this;
                    return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                        data = res.data;
                        ...
                        this.allSermons.forEach(function (e, i) {
                            _this.newFeeds.push(_this.allSermons[i]);
                        });
                        if (this.allSermons.length != 0) {
                            // filtering out sermon with the sermon id from url
                            this.allSermons = this.allSermons.filter(function (x) { return x.enclosure.link != ''; });
                            // this.newFeeds = this.allSermons;
                            this.allSermons.map(function (y) {
            ...
            if (this.page <= this.total_page_in_pagination) {
                ...
                this.sermonsService
                    ...
                    if (res.audio) {
                        ...
                        res.audio.forEach(function (element) {
                            _this.allSermons.push(element);
                        });
                        if (_this.allSermons.length != 0) {
                            // filtering out data which have URL
                            _this.allSermons = _this.allSermons.filter(function (x) { return x.enclosure["@attributes"].url != ""; });
                            _this.allSermons.map(function (y) {
                                y["audio_link"] = _this.dom.bypassSecurityTrustUrl(y.enclosure["@attributes"].url);
                                y["share_link"] = y.enclosure["@attributes"].url;
                                y["isPlaying"] = false;
                                y['isDownloaded'] = false;
    ...
    AudioArchivePage.prototype.getXMLRss = function (sermon_url) {
        var _this = this;
        this.loader.presentLoading().then(function () {
            var self = _this;
            // fetching data through service
            _this.sermonsService.getrss(sermon_url).subscribe(function (res) {
                ...
                if (res && res.status === true && res.data && res.data.status === "ok") {
                    ...
                    if (self.allSermons.length != 0) {
                        // filtering out data which have URL
                        self.allSermons = self.allSermons.filter(function (x) { return x.enclosure.link != ""; });
                        self.allSermons.map(function (y) {
                            y["a_link"] = y.link;
                        });
                        // Saving all feeds in database through API call in service
                ...
                _this.loader.stopLoading();
                _this.toast.presentToastWithOptions(_this.alertMessage
                    ? _this.alertMessage.AudioURLNotWorking
                    : "Audio URL not working, Try Again", _this.alertMessage
                    ? _this.alertMessage.WarningMessage
                    : "Warning Message", "warning");
            });
        });
    };
    AudioArchivePage.prototype.getSermonsFromWordpress = function (base_url) {
        var _this = this;
        this.loader.presentLoading().then(function () { });
        var self = this;
        if (this.external_server) {
            ...
            this.sermonsService.getFeed(_body).subscribe(function (res) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
                var all, sermons, x, obj, body_1;
                var _this = this;
                return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                    this.fetchMethod = 'wrodpress';
                    all = res.result.channel.item;
                    sermons = [];
                    if (all.length != 0) {
                        // filtering out sermon with the sermon id from url
                        ...
                        self.sermonsService
                            .create_Audio_Feeds_array(body_1)
                            .subscribe(function (res) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
                            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                                if (res && res.status) {
                                    // if loop iteration is equal to length of data, its time to fetch what we have saved
            ...
            // fetching sermon data through service
            this.sermonsService.getAllSermons(base_url, this.page).subscribe(function (res) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
                var data, body;
                var _this = this;
                return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                    data = res.data;
    ...
    AudioArchivePage.prototype.load_feed = function () {
        var _this = this;
        var self = this;
                ...
                if (new_feed.length > 0) {
                    ...
                    new_feed.map(function (x) {
                        ...
                        x.comments.map(function (item) {
                            ...
                            var mytime = moment__WEBPACK_IMPORTED_MODULE_13__(item.comment_time);
                        ...
                        if (_this.external_server) {
                            ...
                            if (audio.length > 0) {
                                ...
                                if (audio[0].audio_link &&
                                    ...
                                    x["audio_link"] = _this.dom.bypassSecurityTrustUrl(audio[0].audio_link);
                                }
                                else if (audio[0].enclosure["@attributes"]["url"] &&
                                    audio[0].enclosure["@attributes"]["url"] != "") {
                                    x["audio_link"] = _this.dom.bypassSecurityTrustUrl(audio[0].enclosure["@attributes"]["url"]);
                                }
                                x["share_link"] = x.link;
                            ...
                            if (audio.length > 0) {
                                ...
                                if (audio[0].audio_link &&
                                    ...
                                    if (audio[0].audio_link instanceof Array) {
                                        x["audio_link"] = _this.dom.bypassSecurityTrustUrl(audio[0].audio_link[0]);
                                    }
                                    else {
                                        x["audio_link"] = _this.dom.bypassSecurityTrustUrl(audio[0].audio_link);
                                    }
    ...
    AudioArchivePage.prototype.checkDownloads = function () {
        //Checking files which already downloaded
        var _this = this;
        if (this.platform.is('android')) {
            this.newFeeds.forEach(function (e, i) {
                _this.file.checkFile(_this.file.externalApplicationStorageDirectory + 'radioBase/', e.name + '.mp3').then(function (result) {
                    _this.newFeeds[i].isDownloaded = result;
                }).catch(function (err) {
                });
            });
        }
            ...
            this.newFeeds.forEach(function (e, i) {
                _this.file.checkFile(_this.file.documentsDirectory + 'radioBase/', e.name + '.mp3').then(function (result) {
                    _this.newFeeds[i].isDownloaded = result;
                }).catch(function (err) {
                });
            });
    ...
    AudioArchivePage.prototype.settingSeek = function () {
        var _this = this;
        this.cardIndex = this.resumableArray.findIndex(function (elem) {
            if (elem.id === _this.page_id) {
                return true;
            }
        });
        if (this.resumableArray[this.cardIndex].seekValues.length < 1) { //check if seekValues array have data or not
            this.newFeeds.forEach(function (e, i) {
                _this.lastSeekArray.push({ value: 0, name: e.name, progress: 0, duration: 0, pubDate: e.pubDate });
                _this.resumableArray.splice(_this.cardIndex, 1, { id: _this.resumableArray[_this.cardIndex].id, title: _this.resumableArray[_this.cardIndex].title, seekValues: _this.lastSeekArray });
            });
        }
        else {
            this.newFeeds.forEach(function (e, i) {
                _this.resumeSeek = _this.resumableArray[_this.cardIndex].seekValues;
                e['progress'] = 0;
                e['duration'] = 0;
                e['value'] = 0;
                _this.resumeSeek.forEach(function (value, ind) {
                ...
                _this.lastSeekArray.push({ value: e['value'], name: e.name, progress: e['progress'], duration: e['duration'], pubDate: e.pubDate });
                _this.resumableArray.splice(_this.cardIndex, 1, { id: _this.resumableArray[_this.cardIndex].id, title: _this.resumableArray[_this.cardIndex].title, seekValues: _this.lastSeekArray });
            });
        }
        localStorage.setItem('resumableValues', JSON.stringify(this.resumableArray));
    };
    AudioArchivePage.prototype.load_rss_feed = function () {
        var _this = this;
        var self = this;
        // Loading all saved Feeds
        self.sermonsService
            ...
            if (res && res.feed) {
                ...
                if (new_feed.length > 0) {
                    ...
                    new_feed.map(function (x) {
                        ...
                        // filtering data matching url and title
                        if (x.name === "") {
                            audio = _this.allSermons.find(function (y) { return y.enclosure["@attributes"].url === x.link; });
                        }
                        else {
                            audio = _this.allSermons.find(function (y) {
                                return y.enclosure["@attributes"].url === x.link &&
                                    y.title === x.name;
                            });
                        }
                        if (audio) {
                            // passing audio url from dom sanitizer for security purpose
                            x["audio_link"] = _this.dom.bypassSecurityTrustUrl(audio.enclosure["@attributes"].url);
                            x["isPlaying"] = false;
                            x['isDownloaded'] = false;
                        }
                        else {
    ...
    AudioArchivePage.prototype.load_xml_rss = function () {
        var _this = this;
        var self = this;
        // Loading all saved Feeds
        self.sermonsService
            ...
            if (res && res.feed) {
                ...
                new_feed.map(function (x) {
                    ...
                    if (audio.length > 0) {
                        // passing audio url from dom sanitizer for security purpose
                        x["audio_link"] = _this.dom.bypassSecurityTrustUrl(audio[0].enclosure.link);
    ...
    AudioArchivePage.prototype.getUser = function () {
        var _this = this;
        ...
        this.storage.get("user").then(function (res) {
            if (res != null && res != "guest") {
                ...
                _this.auth = res.auth;
                _this.gender = res.user.gender;
                _this.age = res.user.birthday;
                _this.user = res;
            }
        });
        this.device_id = localStorage.getItem('device_id');
    };
    AudioArchivePage.prototype.create_comment = function (feed_id, index) {
        var _this = this;
    ...
    AudioArchivePage.prototype.gotobackPage = function () {
        this.location.back();
    };
    AudioArchivePage.prototype.setAnalytics = function () {
        var dev_id = "";
        var device = "web";
        ...
        var duration = moment__WEBPACK_IMPORTED_MODULE_13__["duration"](end_time_analytics.diff(this.start_time_analytics))
            .asSeconds();
        // preparing user data
        var userId = this.isUserLoggedIn ? this.account_donor_id : "guest";
    ...
    AudioArchivePage.prototype.isEmpty = function (obj) {
        return Object.keys(obj).length === 0;
    };
    AudioArchivePage.prototype.ionViewDidLeave = function () {
        this.setAnalytics();
        this.event_provider.setAudioPlayer(true);
    };
    AudioArchivePage.prototype.ionViewDidEnter = function () {
        // setting start time for analytics
        this.start_time_analytics = moment__WEBPACK_IMPORTED_MODULE_13__();
    };
    AudioArchivePage.prototype.presentPopover = function (ev, index) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var popover;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                    ...
                    case 1:
                        ...
                        popover.onDidDismiss().then(function (res) {
                            if (res.data && res.data.event) {
                                if (_this.comments[index] === undefined) {
                                    _this.comments[index] = "";
                                }
                                _this.comments[index] += res.data.event.char;
                            }
                        });
    ...
    AudioArchivePage.prototype.download = function (data, index) {
        var _this = this;
        // console.log('downlaoding: ', data,);
        var url = data.audio_link['changingThisBreaksApplicationSecurity'];
        this.index = index;
        ...
        if (this.isDownloadActive[index] === true) {
            ...
            this.downloadService.addFileToDownloads(dataBody).subscribe(function (res) {
                if (res.status) {
            ...
            // }
            if (this.platform.is('android')) {
                this.file.checkDir(this.file.externalApplicationStorageDirectory, 'radioBase').then(function (e) {
                    if (e === true) {
                        _this.file.checkFile(_this.file.externalApplicationStorageDirectory + 'radioBase/', data.name + '.mp3').then(function (result) {
                            _this.toast.presentToastWithOptions('File Already Downloaded', 'Sorry', 'warning');
                            _this.isDownloadActive[index] = false;
                        }).catch(function (err) {
                            _this.isDownloading[index] = true;
                            _this.toast.presentToastWithOptions('Downloading is in progress', 'Please Wait...', 'success');
                            _this.fileTransfer.download(url, _this.file.externalApplicationStorageDirectory + 'radioBase/' + data.name + '.mp3').then(function (entry) {
                                if (entry) {
                                    _this.isDownloading[index] = false;
                                    _this.toast.presentToastWithOptions('Download Succeed', 'Thanks', 'success');
                                    _this.newFeeds[index].isDownloaded = true;
                                    _this.isDownloadActive[index] = false;
                                }
                            }).catch(function (err) {
                                _this.isDownloading[index] = false;
                                _this.toast.presentToastWithOptions(err, 'Sorry', 'danger');
                            });
                        });
                    }
                }).catch(function (err) {
                    _this.file.createDir(_this.file.externalApplicationStorageDirectory, 'radioBase', false).then(function (e) {
                        _this.isDownloading[index] = true;
                        _this.toast.presentToastWithOptions('Downloading is in progress', 'Please Wait...', 'success');
                        _this.fileTransfer.download(url, _this.file.externalApplicationStorageDirectory + 'radioBase/' + data.name + '.mp3').then(function (entry) {
                ...
                this.file.checkDir(this.file.documentsDirectory, 'radioBase').then(function (e) {
                    if (e === true) {
                        _this.file.checkFile(_this.file.documentsDirectory + 'radioBase/', data.name + '.mp3').then(function (result) {
                            _this.toast.presentToastWithOptions('File Already Downloaded', 'Sorry', 'warning');
                            _this.isDownloadActive[index] = false;
                        }).catch(function (err) {
                            _this.isDownloading[index] = true;
                            _this.toast.presentToastWithOptions('Downloading is in progress', 'Please Wait...', 'success');
                            _this.fileTransfer.download(url, _this.file.documentsDirectory + 'radioBase/' + data.name + '.mp3').then(function (entry) {
                                if (entry) {
                                    _this.isDownloading[index] = false;
                                    _this.toast.presentToastWithOptions('Download Succeed', 'Thanks', 'success');
                                    _this.newFeeds[index].isDownloaded = true;
                    ...
                    _this.file.createDir(_this.file.documentsDirectory, 'radioBase', false).then(function (e) {
                        _this.isDownloading[index] = true;
                        _this.toast.presentToastWithOptions('Downloading is in progress', 'Please Wait...', 'success');
                        _this.fileTransfer.download(url, _this.file.documentsDirectory + 'radioBase/' + data.name + '.mp3').then(function (entry) {
                            if (entry) { //If file downloaded
    ...
    AudioArchivePage.prototype.commentsActive = function (index) {
        this.index = index;
    ...
    AudioArchivePage.prototype.successCallback = function (result) {
        console.log(result); // true - enabled, false - disabled
    };
    AudioArchivePage.prototype.errorCallback = function (error) {
        console.log(error);
    };
    AudioArchivePage.prototype.castingModel = function (audio, session) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var i, modal;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        _a.label = 1;
                    case 1:
                        if (!(i < this.newFeeds.length)) return [3 /*break*/, 5];
                        if (!(this.newFeeds[i].audio_link["changingThisBreaksApplicationSecurity"] ==
                            audio["changingThisBreaksApplicationSecurity"])) return [3 /*break*/, 4];
    ...
    //     return await modal.present();
    // }
    AudioArchivePage.prototype.castAud = function (url, index) {
        var _this = this;
        ...
        if (url["changingThisBreaksApplicationSecurity"] && url["changingThisBreaksApplicationSecurity"].includes("soundcloud.com")) {
            ...
            setTimeout(function () {
                console.log("else");
                if ((_this.casting.session && _this.casting.session != '') && (_this.casting._media || _this.casting._media !== '') && (_this.casting.playURl === url)) {
                    _this.casting.requestSession(url, "audio-archieve", index);
                }
                else if ((_this.casting.session && _this.casting.session != '') && (_this.casting._media || _this.casting._media !== '') && (_this.casting.playURl !== url)) {
                    _this.casting.changeAudio(url, "audio-archieve", index);
                }
                else {
                    _this.casting.requestSession(url, "audio-archieve", index);
                }
    ...
    AudioArchivePage.ctorParameters = function () { return [
...
//# sourceMappingURL=audio-archive-audio-archive-module-es5.js.map