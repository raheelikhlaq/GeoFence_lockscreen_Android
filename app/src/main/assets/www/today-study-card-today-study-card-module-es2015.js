...
let TodayStudyCardPageModule = class TodayStudyCardPageModule {
};
...
let TodayStudyCardPage = class TodayStudyCardPage {
    constructor(translate, loader, toast, apiService, route, event_provider, router, storage, idSer, audioService, service, downloadService, socialSharing, location, modalController, actionSheetController, _bibleService, platform) {
    ...
    getTodayStudy(scheduled_date) {
        ...
        this.apiService.getTodayStudy(body).subscribe((res) => {
            if (res) {
                ...
                this.sevenDaysStudy = this.sevenDaysStudy.concat(obj);
                ...
                console.log(this.sevenDaysStudy);
                // var  read_audios:any=[];
    ...
    openModal(item) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (this.is_cordova) {
                let share_data = '"' + item.name + '" ' + item.description + ' ' + item.audioUrl;
    ...
    ReadTheBiblePassage(item) {
        console.log(item);
        let item2 = this.books.find(x => x.id === item.book_id);
        console.log(item2);
        if (item2) {
            let data = {
                queryParams: {
    ...
    playTodayStudy(isplay, audio, index) {
        ...
        if (this.audioService.audioUrl == audiourl) {
            ...
            setTimeout(() => {
                ...
                audio.seekto > 0 ? this.audioService.startAudio(audiourl, audio.seekto) : this.audioService.startAudio(audiourl);
            }, 500);
        }
    }
    goback() {
    ...
    studyRead(read) {
        ...
        if (listenData) {
            ...
            if (idx > -1) {
                var idxs = this.sevenDaysStudy.findIndex(x => x.uuid == listenData[idx].id);
                if (idxs > -1) {
...
TodayStudyCardPage.ctorParameters = () => [
...
//# sourceMappingURL=today-study-card-today-study-card-module-es2015.js.map