...
let EventDetailPage = class EventDetailPage {
    constructor(storage, route, loader, eventsService, toast, iab, location, router, appService, common, dom, translate, platform, event_provider) {
    ...
    getDBEvent() {
        ...
        this.eventsService.getCMSEvent(body).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            let events = res['result'];
            this.event_detail = res['event'];
            this.show_map = this.event_detail.show_map === 'true';
            // Removing extra Special Characters from Description
            let text = this.event_detail.description;
            ...
            let sum = this.event_detail.title;
            ...
            // Getting City and Country seperately
            let obj = this.event_detail;
            ...
            for (let j = 0; j < keys.length; j++) {
                ...
                if (keys[j].includes("featuredimageurl")) {
                    ...
                    if (this.event_photo !== "" && this.event_photo !== null && this.event_photo.includes("s3.wasabisys.com") && Object(_angular_common__WEBPACK_IMPORTED_MODULE_8__["formatDate"])(this.event_detail.created_at, 'yyyy-MM-dd', 'en_US') >= '2023-05-29') {
                        let abc = this.event_photo.split("/");
                        ...
                        let new_link = this.event_photo.replace(folder_name, this.deviceType);
            ...
            if (typeof this.event_detail['DTSTART'] === 'object') {
                ...
                Object.keys(this.event_detail).forEach(key => {
            ...
            let _start = moment__WEBPACK_IMPORTED_MODULE_13__(this.event_detail.start_date);
            let _end = moment__WEBPACK_IMPORTED_MODULE_13__(this.event_detail.end_date);
            let x = _start.format('YYYY-MM-DD HH:mm:ss').toString();
            let y = _end.format('YYYY-MM-DD HH:mm:ss').toString();
            let start = moment__WEBPACK_IMPORTED_MODULE_13__(x);
            let end = moment__WEBPACK_IMPORTED_MODULE_13__(y);
    ...
    getEvent() {
        if (this.is_event_url == 'true') {
            ...
            this.eventsService.getEvents(this.event_url).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (res.data) {
                    let events = yield res.data.VEVENT;
                    this.event_detail = events[0];
                    // Removing extra Special Characters from Description
                    let ev_des;
                    let text = this.event_detail.DESCRIPTION.toString();
                    ev_des = text.replace(/\\n/g, '');
                    ev_des = ev_des.replace(/\\/g, '');
                    let code = ev_des;
                    if (code.search('<a') != -1 && this.platform.is('ios')) {
                        do {
                            let link = code.match(/data-cke-saved-href="(.*?)"/) ? code.match(/data-cke-saved-href="(.*?)"/) : code.match(/href="(.*?)"/);
                    ...
                    // editing Venue Name html
                    let venue_name;
                    venue_name = this.event_detail.VENUENAME;
                    let code1 = venue_name;
                    if (code1 && code1.search('<a') != -1 && this.platform.is('ios')) {
                        do {
                            let link = code1.match(/data-cke-saved-href="(.*?)"/) ? code1.match(/data-cke-saved-href="(.*?)"/) : code1.match(/href="(.*?)"/);
            ...
            if (this.event_mode === 'dashboard') {
                if (this.event_url.includes("calendar.google.com")) {
                    ...
                    this.eventsService.oneEvent(body).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                    ...
                    if (this.external_server) {
                        ...
                        this.eventsService.getDashboardEvent(_body).subscribe((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                            let events = res['result'];
                            let filtered = events.filter(x => md5(x.UID) === this.event_id);
                            this.event_detail = yield filtered[0];
                            // Removing extra Special Characters from Description
                            let ev_des;
                            let text = this.event_detail.DESCRIPTION.toString();
                            ev_des = text.replace(/\\n/g, '');
                ...
                this.eventsService.getEventsFromChurchId(this.ch_id).subscribe((res) => {
                    if (res) {
                        ...
                        this.church_event_detail = filtered[0];
                        if (this.church_event_detail.photo) {
                            this.event_photo = this.church_event_detail.photo.replace(/ /g, "%20");
                        }
                        if (this.event_photo !== "" && this.event_photo !== null && this.event_photo.includes("s3.wasabisys.com") && Object(_angular_common__WEBPACK_IMPORTED_MODULE_8__["formatDate"])(this.church_event_detail.created_at, 'yyyy-MM-dd', 'en_US') >= '2023-05-29') {
                            let abc = this.event_photo.split("/");
                            //  let img_name = abc[abc.length-1];
                            let folder_name = abc[abc.length - 2];
                            if (this.platform.is("mobile")) {
    ...
    getlocation(data) {
        let re = /\d{5}-\d{4}|\d{5}|[A-Z]\d[A-Z] \d[A-Z]\d/;
        let input = data.substring(5);
        let match = re.exec(input);
        if (match !== null) {
            if (match[0] !== "") {
                let postalcode = match[0];
                this.eventsService.getMapLocation(data, postalcode).subscribe((res) => {
                    this.chLat = res.results[0].geometry.location.lat;
                    this.chLong = res.results[0].geometry.location.lng;
                });
            }
            else {
