...
var JourneyPage = /** @class */ (function () {
    ...
    function JourneyPage(translate, loader, toast, apiService, route, event_provider, router, storage, location, platform, socialSharing, idSer, service, _bibleService, actionSheetController, audioService, downloadService, modalController) {
    ...
    JourneyPage.prototype.setDownloadMode = function (event) {
        this.autoDownload = event.detail.checked;
        localStorage.setItem("setAutoDownload", event.detail.checked);
    };
    JourneyPage.prototype.ionViewDidEnter = function () {
        var _this = this;
        ...
        this.storage.get('app_language').then(function (res) {
            ...
            _this.getCMPBooks(res.bible_id);
            if (res.bible_id) {
        ...
        this.LastStudy();
        setTimeout(function () {
            console.log(_this.widgetsContent.nativeElement);
            // this.widgetsContent.nativeElement.scrollTo({ left: (this.widgetsContent.nativeElement.scrollLeft - 500), behavior: 'smooth' });
            _this.widgetsContent.nativeElement.scrollLeft += 350;
        }, 1000);
    };
    JourneyPage.prototype.getUser = function () {
        var _this = this;
        this.storage.get('user').then(function (res) {
            if (res && res.user.id) {
                _this.account_donor_id = res.user.id;
                _this.is_user_logged_in = true;
            }
            else {
                _this.is_user_logged_in = false;
            }
        });
    };
    JourneyPage.prototype.getTodayStudy = function (scheduled_date) {
        var _this = this;
        ...
        this.loader.presentLoading().then(function () {
            _this.apiService.getTodayStudy(body).subscribe(function (res) {
                if (res) {
                    _this.todayStudy = res;
                    _this.book_id = res.bible_book;
                    _this.book_name = res.bible_book;
                    _this.book_name_eng = res.bible_book;
                    _this.organization = res.organization;
                    ...
                    _this.description = res.description,
                        _this.audioUrl = src_environments_environment__WEBPACK_IMPORTED_MODULE_8__["environment"].cmp_audioLink + _this.id;
                    _this.todayStudy_img = res.art_url;
                    _this.today = res;
                    _this.audio_format = res.audio_format;
                    _this.cmp_book_id = res.cmp_book_id;
                    _this.start_chapter = res.bible_chapter_start;
                    _this.end_chapter = res.bible_chapter_end;
                    _this.start_verse = res.bible_verse_start;
                    _this.end_verse = res.bible_verse_end;
                    _this.audio_format = res.audio_format;
                    _this.getTtbLanguageAudio();
                    _this.loader.stopLoading();
                    setTimeout(function () {
                        if ((_this.autoDownload == true || _this.autoDownload == "true") && (scheduled_date.getDay() == _this.todayDate)) {
                            var res = _this.downloadService.downloadView(_this.today);
                            console.log(res, "caluesssss");
                            if (!res) {
                                _this.downloadService.downloadsLocalSave(_this.today, "study");
                            }
                        }
                    }, 2000);
                }
    ...
    JourneyPage.prototype.getTtbLanguageAudio = function () {
    ...
    JourneyPage.prototype.getTeachingImagesCache = function (book_name) {
    ...
    JourneyPage.prototype.getLastWeeksDate = function (days) {
        var now = new Date();
        return new Date(now.getFullYear(), now.getMonth(), now.getDate() - (6 - days));
    };
    JourneyPage.prototype.changeStudyOrder = function (order) {
        this.study_order = order;
    };
    JourneyPage.prototype.getBibleBooks = function (bible_id) {
        var _this = this;
        this.apiService.getLanguageBooksListTeachings(bible_id).subscribe(function (res) {
            ...
            if (res) {
                ...
                booksss.map(function (x) {
                    x['book_name'] = x.name;
                    x['book_id'] = x.id;
                    var badge_image = _this.getTeachingImagesCache(x['book_name']);
                    x['no_of_teachings'] = 0;
                    x['teachingDetail'] = {};
                    ...
                    _this.getTtbLanguageAudios(x);
                });
                var orderBooks = booksss.sort(function (a, b) { return a.bookOrder - b.bookOrder; });
                _this.books = orderBooks;
                _this.new_testament = _this.books.filter(function (x) {
                    return x['testament'] == 'N';
                });
                _this.old_testament = _this.books.filter(function (x) {
                    return x['testament'] == 'O';
                });
                ...
                //   )
                listenStudy.forEach(function (element) {
                    if (element.book_id) {
                        console.log(element);
                        var idx = _this.books.findIndex(function (x) { return x.book_id == element.book_id; });
                        console.log(idx);
                        if (idx > -1 && (_this.history_testament.length == 0 || (_this.history_testament.length > 0 && _this.history_testament.find(function (x) { return x.book_id != element.book_id; })))) {
                            var data = _this.books[idx];
                            _this.history_testament.push(data);
                            console.log(_this.history_testament);
                        }
                    }
                });
                ...
                setTimeout(function () {
                    ...
                    _this.Prophecy = _this.books.filter(function (n) {
                        return prosFilt.find(function (x) { return x.name == n.name; });
                    });
                    _this.History = _this.books.filter(function (n) {
                        return historyFilt.find(function (x) { return x.name == n.name; });
                    });
                    _this.Gospels = _this.books.filter(function (n) {
                        return gospelFilt.find(function (x) { return x.name == n.name; });
                    });
                    setTimeout(function () {
                        console.log(_this.Prophecy, _this.History, _this.Gospels, "myfilterrrrrrr");
                    }, 2000);
                }, 3000);
            }
        });
    };
    JourneyPage.prototype.getTtbLanguageAudios = function (book) {
            ...
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                this.storage.get('app_language').then(function (res) {
                    if (res.bible_id) {
                        ...
                        _this.apiService.getTtbLanguageAudios(body).subscribe(function (res) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
                            var filterData, progress;
                            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                                if (res && res.length) {
                                    book.no_of_teachings = res.length;
                                    book.teachingDetail = res;
                                    if (this.listenStudy.length > 0) {
                                        filterData = this.listenStudy.filter(function (x) { return (x.book_id == book.book_id) && x.completeListner == true; });
                                        progress = (filterData.length / book.no_of_teachings) * 100 || 0;
    ...
    JourneyPage.prototype.openBibleVideos = function (book_id, book_name, book_name_eng) {
    ...
    JourneyPage.prototype.onLogin = function () {
    ...
    JourneyPage.prototype.gotobackPage = function () {
        this.location.back();
    };
    JourneyPage.prototype.playTodayStudy = function (isplay) {
        var _this = this;
        if (this.audioService.audioUrl == this.audioUrl) {
            ...
            setTimeout(function () {
                var seek = 0;
                if (_this.progressData && _this.progressData.listenDuration) {
                    var timeArray = _this.progressData.listenDuration.split(':');
                ...
                _this.audioService.audioIndex = -1;
                _this.service.image = _this.todayStudy_img;
                _this.audioData.image = _this.todayStudy_img;
                _this.audioService.title = _this.audioData.title;
                _this.audioService.isAudioPlayShows = !isplay;
                _this.audioService.image = _this.todayStudy_img;
                _this.service.audioData = _this.todayStudy;
                _this.audioService.organization = _this.organization;
                _this.audioService.is_today_study = true;
                _this.audioService.study_image = _this.todayStudy_img;
                setTimeout(function () {
                    seek > 0 ? _this.audioService.startAudio(_this.audioUrl, seek) : _this.audioService.startAudio(_this.audioUrl);
                }, 400);
            }, 500);
        }
    };
    JourneyPage.prototype.shareURL = function (audio) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var actionSheet;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
    ...
    JourneyPage.prototype.callingAgain = function () {
        var _this = this;
        setTimeout(function () {
            _this.LastStudy();
        }, 800);
    };
    JourneyPage.prototype.LastStudy = function () {
        var _this = this;
        ...
        if (this.listenStudy && this.listenStudy.length > 0) {
            this.storage.get('app_language').then(function (res) {
                if (res.bible_id) {
                    var body = JSON.stringify({
                        language: res.code == 'en' ? 'en_US' : res.code == 'zh_HantT' ? 'zh_Hant' : localStorage.getItem('app_language'),
                        bible_book: _this.listenStudy[_this.listenStudy.length - 1].book_id,
                        audio_format: "Daily"
                    });
                    _this.apiService.getTtbLanguageAudios(body).subscribe(function (res) {
                        if (res && res.length) {
                            _this.setCmpData(res);
                        }
                        // else{  
    ...
    JourneyPage.prototype.setCmpData = function (res) {
        var _this = this;
        var ttb_audios = res;
        ttb_audios.map(function (x) {
        ...
        ttb_audios.forEach(function (audio) {
            audio.duraionInMints = _this.intoMinutes(audio.audio_duration);
            audio.listner = false;
        ...
        var idx = ttb_audios.findIndex(function (x) { return x.uuid == _this.listenStudy[_this.listenStudy.length - 1].id; });
        this.index = idx > -1 ? idx : 0;
    };
    JourneyPage.prototype.intoMinutes = function (second) {
    ...
    JourneyPage.prototype.seek = function () {
        var _this = this;
    ...
    JourneyPage.prototype.lastListenAudio = function () {
        var _this = this;
        this.audioService.clear();
        setTimeout(function () {
            _this.audioService.clear();
            var audioData = {};
            audioData = _this.ttb_audios[_this.index];
            var audio = src_environments_environment__WEBPACK_IMPORTED_MODULE_8__["environment"].cmp_audioLink + _this.ttb_audios[_this.index].uuid;
            _this.audioService.study_ttbAudios = new Array(_this.ttb_audios.length).fill(false);
            _this.audioService.audioIndex = _this.index;
            _this.audioService.study_ttbAudios[_this.index] = true;
            audioData.book_id = _this.listenStudy[_this.listenStudy.length - 1].book_id;
            _this.audioService.fromPage = 'ism-study';
            _this.audioService.title = audioData.name;
            _this.audioService.image = _this.ttb_audios[_this.index].art_url;
            _this.audioService.tt_all_audios = _this.ttb_audios;
            _this.audioService.liveStreaming = false;
            audioData.title = audioData.name;
            audioData.image = _this.ttb_audios[_this.index].art_url;
            _this.service.method = 'ism-study';
            _this.audioService.index = _this.index;
            _this.audioService.audioUrl = audio;
            _this.service.audioData = audioData;
            _this.audioService.bibleBook_name = _this.listenStudy[_this.listenStudy.length - 1].complete_bookname;
            _this.audioService.fromPage = 'ism-study';
            _this.audioService.startAudio(audio);
        }, 500);
    };
    JourneyPage.prototype.download = function () {
        var _this = this;
        if (this.is_user_logged_in) {
            ...
            this.downloadService.addFileToDownloads(dataBody).subscribe(function (res) {
                if (res.status) {
                    ...
                    _this.toast.presentToastWithOptions(alertMessage, '', 'Success');
                }
            });
        }
    };
    JourneyPage.prototype.getCMPBooks = function (bible_id) {
        var _this = this;
        var self = this;
        var bible_id2 = bible_id;
        var code = localStorage.getItem('app_language') == 'en' ?
            localStorage.getItem('app_language') + '_US' : localStorage.getItem('app_language') == 'zh_HantT' ? 'zh_Hant' : localStorage.getItem('app_language');
        this._bibleService.getCMPbibleBooks(bible_id2, code).subscribe(function (res) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
            var data_1;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                if (res) {
                    data_1 = [];
    ...
    JourneyPage.prototype.downloadCompoent = function () {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var modal;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                    ...
                    case 1:
                        ...
                        modal.onDidDismiss()
                            ...
                            if (user && user.data === 'passenger') {
                                _this.ReadTheBiblePassage(_this.today);
                            }
                        });
    ...
    JourneyPage.prototype.error = function (e, r) {
        console.log(e, r, "this.is error");
    };
    JourneyPage.prototype.lastListenAudioReview = function (getData) {
        console.log(getData);
        if (getData.name == this.audioService.title) {
            ...
            if (getData.book_id ? true : getData.cmp_book_id) {
                ...
                var filter = this.books.find(function (x) { return x.book_id == (getData.book_id ? getData.book_id : getData.cmp_book_id); });
                if (filter) {
                    var idx = filter.teachingDetail.findIndex(function (x) { return x.uuid == getData.uuid; });
                    if (idx > -1) {
                        var audioData = {};
    ...
    JourneyPage.prototype.ReadTheBiblePassage = function (item) {
        var item2 = this.cmp_books.find(function (x) { return x.id === item.cmp_book_id; });
        console.log(item2);
        if (item2) {
            var data = {
                queryParams: {
    ...
    JourneyPage.prototype.studyRead = function (read) {
        var receeivedData = {};
    ...
    JourneyPage.prototype.getTodayStudyFOrCompleted = function (scheduled_date) {
        var _this = this;
        var body = JSON.stringify({
            language: localStorage.getItem('app_language') == 'en' ?
                localStorage.getItem('app_language') + '_US' : localStorage.getItem('app_language') == 'zh_HantT' ? 'zh_Hant' : localStorage.getItem('app_language'),
    ...
    JourneyPage.ctorParameters = function () { return [
...
//# sourceMappingURL=journey-journey-module-es5.js.map