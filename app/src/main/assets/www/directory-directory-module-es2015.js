...
let DirectoryPage = class DirectoryPage {
    constructor(platform, route, loader, storage, sermonsService, dom, router, location, common, toast, apiService, idService, translate) {
    ...
    get_directory() {
        ...
        this.apiService.get_directory(this.link_code).subscribe((res) => {
            if (res.directory) {
                let campus_id = res.directory.campus_id;
                let church_id = res.directory.church_id;
                const body = JSON.stringify({
                    church_id: church_id,
                    campus_id: +campus_id,
                });
                this.apiService.get_directories(body).subscribe((dir) => {
                    if (dir.status) {
                        this.rows = dir.directory;
                        // setting data
                        this.rows.map(x => x['name'] = x.first_last_name);
                        this.rows.map(x => x['zip'] = x.postal_code);
                    }
                });
            }
        });
