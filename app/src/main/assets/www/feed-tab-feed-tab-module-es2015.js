...
let FeedTabPageModule = class FeedTabPageModule {
};
...
let FeedTabPage = class FeedTabPage {
    constructor(idService, route, loader, storage, sermonsService, router, eventsService, apiService, dom, auth, toast, datePipe, appService, location, analytics, formBuilder, platform, device, translate, eventProviderService, iab, popoverController, wordPressSerice, actionSheetController, camera, modalCtrl, common) {
    ...
    ngOnInit() {
        ...
        let appPages = JSON.parse(localStorage.getItem('church_data')).apppages;
        if (appPages) {
            let event_page = appPages.filter(x => x.name === 'Events');
            if (event_page.length > 0) {
                this.event_page_exist = true;
            }
            // getting pagename for analytics
            let feed_page = appPages.find(x => x.name === 'Feed');
            if (feed_page) {
    ...
    getTheme() {
        this.eventProviderService.getChurchData.subscribe(theme => {
            if (!(Object.keys(theme).length === 0)) {
                this.ch_id = theme.church.ch_id;
                this.theme = theme.church;
                this.getUser();
                this.getNews(this.ch_id);
                // getting pagename for analytics
                let feed_page = theme.apppages.find(x => x.name === 'Feed');
                if (feed_page) {
                    this.page_name = feed_page.name;
                    this.custom_name = feed_page.title;
                    var id = feed_page.id;
                    let app_pages = theme.apppages.find(x => x.id === id);
    ...
    getNews(ch_id) {
        ...
        this.loader.presentLoading().then(() => {
            this.apiService.get_news_wall(ch_id).subscribe((res) => {
                this.loader.stopLoading();
                if (res.status) {
                    this.news = res.newswall;
                    if (this.news.length > 0) {
                        this.news.map(x => {
                            if (x.photo_profile.includes("https://") || x.photo_profile.includes("http://")) {
                                ...
                                x.photo_profile = x.photo_profile ? this.dom.bypassSecurityTrustUrl(this.IMAGE_BASE_URL + x.photo_profile) : '';
                            }
                            if (x.attachment) {
                                ...
                                if (x.attachment !== "" && x.attachment !== null && x.attachment.includes("s3.wasabisys.com") && x['video'] === false && Object(_angular_common__WEBPACK_IMPORTED_MODULE_13__["formatDate"])(x.created_at, 'yyyy-MM-dd', 'en_US') >= '2023-05-29') {
                                    ...
                                    let new_link = x.attachment.replace(folder_name, this.deviceType);
                                ...
                                x.attachment = this.dom.bypassSecurityTrustResourceUrl(x.attachment);
                            }
                            // reversing news data so that latest news comes on top
                            x.comments = x.comments.slice().reverse();
                            x.comments.map((item) => {
                                try {
                                    item.photo_profile = item.photo_profile ? this.dom.bypassSecurityTrustUrl(item.photo_profile) : '';
                                    item.comments = JSON.parse(item.comments);
                                }
                                catch (_a) {
                                    item.comments = item.comments;
                                }
                                let localTimeZone = moment__WEBPACK_IMPORTED_MODULE_14__["tz"].guess(true);
                            ...
                            if (x.created_at) {
                                let mytime = moment__WEBPACK_IMPORTED_MODULE_14__(x.created_at);
                                let time_diff = moment__WEBPACK_IMPORTED_MODULE_14__().utcOffset(); // in minutes
                                if (time_diff != 0) {
                                    x.created_at = moment__WEBPACK_IMPORTED_MODULE_14__(mytime).add((time_diff), 'minutes');
                                    x.created_at = this.appService.getDateTimeFormat(x.created_at);
                                }
                                else {
                                    x.created_at = this.appService.getDateTimeFormat(x.created_at);
                                }
                        ...
                        let pinned = this.news.filter(x => x.pin === 'Yes');
                        let unpinned = this.news.filter(x => x.pin !== 'Yes');
                        this.news = pinned.reverse().concat(unpinned.reverse());
                        //========= posting PINNED news on TOP =================
                        this.news.forEach((_new) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                            _new.description = _new.description != null ? yield this.dom.bypassSecurityTrustHtml(this.urlify(_new.description)) : '';
                        }));
                    }
                }
                this.loader.stopLoading();
    ...
    createComment(i) {
        if (this.isUserLoggedIn) {
            ...
            let live = new Date().toString();
            live = this.datePipe.transform(live, 'MMM d, y h:mm a');
            let date = new Date(live);
            ...
            // saving comment
            this.apiService.create_newsWall_comment(body).subscribe((res) => {
                this.comment_text[i] = '';
                this.ngOnInit();
            }, (err) => {
    ...
    createLike(i, ev) {
        ...
        if (this.isUserLoggedIn) {
            ...
            // saving LIKE data
            this.apiService.create_newsWall_like(body).subscribe((res) => {
                if (res.status) {
                    this.ngOnInit();
                }
                else {
                    this.toast.presentToastWithOptions((this.alertMessage ? this.alertMessage.already_liked : 'You already Liked!'), (this.alertMessage ? this.alertMessage.Info : 'Info'), 'success');
                    return;
                }
            }, (err) => {
                // if (err.error.http_code === 401){
    ...
    onFileChanged(event) {
        ...
        reader.onload = () => {
            ...
            if (this.attachment.includes('data:video')) {
    ...
    urlify(text) {
        ...
        if (this.platform.is('cordova')) {
            ...
            text.replace(urlRegex, (url) => {
                str = '';
                str = text.replace(urlRegex, `<a href="${url}" target="_blank" onclick="openInNewBrowser('${url}', this)">${url}</a>`);
            });
            return str;
        }
        else {
    ...
    getPicture(opts) {
        this.camera.getPicture(opts).then((imageData) => {
            ...
            this.attachment = imageData ? imageData : "";
            if (this.attachment.includes('data:video')) {
                this.mime_type = 'video';
            }
            else {
        ...
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            modal.onDidDismiss().then(res => {
                if (res.data.selected_image) {
                    let image_data = res.data.selected_image;
                    this.attachment = image_data.url;
                    // this.profile_picture_file = image_data.filename;
                    this.type = 'photo_library';
                    this.mime_type = 'image';
                }
            });
    ...
    postNews() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            if (this.isUserLoggedIn) {
                if (this.news_input) {
                    this.loader.presentLoading().then(res => { });
                    if (this.attachment && this.type !== 'photo_library') {
                        let data = {
                            file: this.attachment
                        };
                        // upload file on wasabi
                        this.wordPressSerice.saveFiles(data).subscribe((res) => {
                            if (res.status) {
                                this.attachment = res.file_name[0];
                                ...
                                this.apiService.save_news(body).subscribe((res) => {
                                    this.loader.stopLoading();
                                    if (res && res.status) {
                                        this.news_input = '';
                                        this.show_share = false;
    ...
    meetTheNeed(i) {
        if (this.isUserLoggedIn) {
            ...
            this.apiService.meet_the_need(body).subscribe((res) => {
                if (res.status) {
                    this.ngOnInit();
                }
                else {
                    this.toast.presentToastWithOptions((this.alertMessage ? this.alertMessage.already_met : 'You already Met!'), (this.alertMessage ? this.alertMessage.Info : 'Info'), 'success');
                    return;
                }
            }, (err) => {
                let message = err.message;
    ...
    ionViewWillEnter() {
        this.eventProviderService.userLogin.subscribe(data => {
            this.getUser();
        });
        this.eventProviderService.userLogout.subscribe(data => {
            this.getUser();
        });
    }
        ...
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            popover.onDidDismiss().then(res => {
                if (res.data && res.data.event) {
                    if (this.comment_text[index] === undefined) {
                        this.comment_text[index] = '';
                    }
                    this.comment_text[index] += res.data.event.char;
                }
            });
...
//# sourceMappingURL=feed-tab-feed-tab-module-es2015.js.map