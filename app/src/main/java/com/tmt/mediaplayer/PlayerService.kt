        ...
        override fun onPlayerCommandRequest(session: MediaSession, controller: MediaSession.ControllerInfo, playerCommand: Int): Int {
            ...
            when (playerCommand) {
              Player.COMMAND_PLAY_PAUSE ->  {
                if (mPlayerState.value == "STOP") {
                  mPlayerState.value = "PLAYING"
                } else if (mPlayerState.value == "PLAYING") {
                  mPlayerState.value = "PAUSE"
                } else if (mPlayerState.value == "PAUSE") {
                  mPlayerState.value = "PLAYING"
                }
...
//            Log.e("Player position service", "" + player.currentPosition)
    ...
    companion object {
        var controllerFuture = MutableLiveData<ListenableFuture<MediaController>>()
        var mPlayerState = MutableLiveData<String>()
        var mSeekPos = MutableLiveData<String>()
        var mDuration = MutableLiveData<String>()
    }
}
